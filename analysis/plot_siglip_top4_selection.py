...
def record_relative_positions(record: dict[str, Any]) -> list[float] | None:
    """Extract all selected frame relative positions from a Test 2 record."""
    if record.get("error"):
        return None
    ...
    if isinstance(selected, list) and selected and isinstance(num_sampled, int) and num_sampled > 0:
        ...
        return [float(index) / float(denom) for index in selected]
    return None
...
def plot_selected_position_line(
    ...
    task_lines: list[tuple[str, np.ndarray, np.ndarray]] = []
    ...
    bin_centers = histogram_line([], NUM_BINS)[0]
    ...
    ax.set_title("SigLIP Top-4 Selected Frame Position Distribution", fontsize=13)
    ...
    out_path = plots_dir / "siglip_top4_selected_position.png"
...
def generate_plots(result_dir: str | Path) -> None:
    ...
    plot_selected_position_line(per_task_positions, plots_dir)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot SigLIP top-4 selected frame relative positions per OVO subset / split / total."
    )
